...
for line in sys.stdin:
	...
	if player_defender in shot_count_dict.keys():
		...
		# update dictionary Values index 0 - hit count( sun score ) and index 1 - count score
...
for player_defender in shot_count_dict.keys():
    #hit_rate = shot_count_dict[player_defender][0]/shot_count_dict[player_defender][1]
    miss_rate = (shot_count_dict[player_defender][1] - shot_count_dict[player_defender][0])/shot_count_dict[player_defender][1]
        
    player, defender = player_defender.split(',')
    #miss_list_byplayer.append([player, defender, miss_rate])
...
# print the most unwanted defender for each player
# player \t most unwanted defender
for key in res:
